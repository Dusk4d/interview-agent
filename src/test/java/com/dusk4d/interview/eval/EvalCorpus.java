    ...
    public static List<Sample> samples() {
        return List.of(
                new Sample("resume-standard.txt", "标准分区简历",
                        ...
                        List.of(
                                new AnswerTier("优秀", """
                                        ...
                                        结果压测下单成功率 99.5%，重复下单为 0。""", 3, 5),
                                new AnswerTier("部分正确", "用 Redis 加锁保证幂等，另外用数据库唯一索引兜底，具体实现记不太清了。", 1, 5),
                ...
                new Sample("resume-tight-headers.txt", "紧凑标题简历",
                        ...
                        List.of(
                                new AnswerTier("优秀", """
                                        这个平台要给课程做推荐，我负责埋点数据模型和日报表。
                                        埋点量大，所以先批量写入再按天分区，单表查询走覆盖索引；
                                        最后写入压力下降 60%，报表延迟从小时级降到分钟级。""", 2, 5),
                                new AnswerTier("部分正确", "我做过课程推荐和埋点，用 MySQL 存数据，做了些优化。", 1, 5),
                ...
                new Sample("resume-numbered-projects.txt", "编号式项目简历",
                        ...
                        List.of(
                                new AnswerTier("优秀", """
                                        秒杀的关键是库存不能超卖。我用 Redis 预扣库存配合 Lua 脚本保证原子性，
                                        再用数据库唯一约束和消息幂等消费做双重保障；
                                        为防热点商品打垮数据库，加了令牌桶限流；压测下单成功率 99.5%。""", 3, 5),
                                new AnswerTier("部分正确", "用 Redis 扣库存，然后用消息队列异步处理订单。", 1, 5),
