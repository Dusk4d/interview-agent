...
> 当前状态：**MVP 全部功能已完成，自动化测试全绿（192 项，含真实 HTTP 端到端、MVP 总验收与打包后进程级冒烟）；并已接入真实本地模型（Ollama qwen3:1.7b）完成实测：问题来源可核验率 100%、结构化输出一次成功率 100%、评分在固定问题与上下文下可完全复现（标准差 0.000）**。
...
| `LlmDiagnosticsTest` | 6 | 「模型未连接」必须说明探测了哪个地址、服务端有哪些模型、该改哪个变量 |
| `ReferenceAnswerTest` | 6 | **参考回答不得编造指标**：无出处的量化指标整段丢弃；序数词不误杀；降级不给参考回答 |
| `RetryAndReplaceTest` | 11 | **重新回答**作废旧分数且报告只算最新一次；**换一道题**不占配额且不重复；换题上限；次数不变量 |
| `ApiExceptionHandlerTest` | 6 | 客户端断连不刷 ERROR 堆栈；非 `/api` 请求不写 JSON 错误体（避免 text/html 转换二次异常）；`/api` 契约不变 |
| `MvpAcceptanceTest` | 14 | **方案书第九节验收标准逐条对应**：全格式导入、事实修正、双模式出题、评分可解释、追问不串题、报告生成、四类失败路径、隐私、跨重启恢复，以及全部反面场景 |

合计 **192 次测试执行全部通过**。其中 `MvpAcceptanceTest` 是「一套跑完即可判断系统是否达标」的门禁测试。
...
| 项目 | 数值 | 来源 |
|---|---|---|
| 自动化测试用例 | 192 项，失败 0 | `scripts\run-tests.ps1` |
...
| 怎么验证质量？ | 192 项自动化测试 + 打包后进程级冒烟 + 真实模型离线评测（LiveEvalMain） | `src/test/java/**`、`scripts/smoke-test.cmd` |
...
└── src/test/java/...           192 项自动化测试 + 评测/冒烟工具
```
