...
> 当前状态：**MVP 全部功能已完成，自动化测试全绿（186 项，含真实 HTTP 端到端、MVP 总验收与打包后进程级冒烟）；并已接入真实本地模型（Ollama qwen3:1.7b）完成实测：问题来源可核验率 100%、结构化输出一次成功率 100%、评分在固定问题与上下文下可完全复现（标准差 0.000）**。
...
合计 **186 次测试执行全部通过**。其中 `MvpAcceptanceTest` 是「一套跑完即可判断系统是否达标」的门禁测试。
...
| 项目 | 数值 | 来源 |
|---|---|---|
| 自动化测试用例 | 186 项，失败 0 | `scripts\run-tests.ps1` |
...
| 怎么验证质量？ | 186 项自动化测试 + 打包后进程级冒烟 + 真实模型离线评测（LiveEvalMain） | `src/test/java/**`、`scripts/smoke-test.cmd` |
...
└── src/test/java/...           186 项自动化测试 + 评测/冒烟工具
```
